...
// dotted landmass: the land polygons are rasterized to a small offscreen
// canvas (one draw call — milliseconds) and the dot grid samples those
// pixels. Far faster than per-point polygon hit-testing, and the data ships
// with the bundle so nothing is fetched from a CDN at runtime.
function buildLandDots(land) {
  ...
  const step = window.innerWidth < 640 ? 2.4 : 1.5
  for (let lat = -80; lat <= 80; lat += step) {
    const n = Math.max(1, Math.round((360 / step) * Math.cos((lat * Math.PI) / 180)))
    for (let i = 0; i < n; i++) {
      const lon = -180 + (360 * i) / n
      const sx = Math.min(W - 1, Math.round(((lon + 180) / 360) * W))
      const sy = Math.min(H - 1, Math.round(((90 - lat) / 180) * H))
      if (px[(sy * W + sx) * 4 + 3] > 0) pts.push(spherePoint([lon, lat]))
    }
  }
...
export default function HeroGlobe() {
  // land data ships in the bundle — no network fetch, no loading state
  const land = useMemo(() => feature(landTopo, landTopo.objects.land), [])
  return (
